...
impl<T: Debug + Ord> Tree<T> {
    ...
    pub fn with_child(mut self: Self, child: Self) -> Self {
    ...
    fn path<'a>(&self, to: &'a T, path: &mut Vec<usize>, idx: usize) -> bool {
        if self.nodes[idx].value == *to {
            true
        } else {
            for child_idx in &self.nodes[idx].children {
                if !path.contains(child_idx) {
                    if self.path(to, path, *child_idx) {
                        path.push(*child_idx);
                        return true;
                    }
                }
